...
class UpsampleBlock(nn.Module):
    ...
    def forward(self, x, y1, y2):
        x = self.upsample(x)
        x = torch.cat([x, y1, y2], dim=1)  # 同时连接两个编码器的跳跃特征
        x = self.block(x)
        return x

# 双编码器-双解码器 U-Net
class DualEncoderDualDecoderUNet(nn.Module):
    def __init__(self, hparams, *args, **kwargs):
        ...
        # 瓶颈层（连接编码器和解码器）
        self.bottleneck = ConvBlock(self.encoder_channels[-1] * 2, self.bottleneck_ch, norm_layer)

        # 高光谱解码器（上采样路径）
        self.spectral_upblocks = nn.ModuleList()
        for i in range(self.n_layers):
            in_ch = (self.bottleneck_ch if i == 0 else self.encoder_channels[self.n_layers - i]) + 2 * self.encoder_channels[self.n_layers - i - 1]
    ...
    def forward(self, captimgs, *args, **kwargs):
        ...
        # 第一个编码器（下采样）
        x1 = self.encoder1_input(captimgs)
        enc1_features = []
        ...
        # 第二个编码器（下采样）
        x2 = self.encoder2_input(captimgs)
        enc2_features = []
        ...
        for i, block in enumerate(self.depth_upblocks):
            x_depth = block(x_depth, enc1_features[self.n_layers - 1 - i], enc2_features[self.n_layers - 1 - i])
...
model = DualEncoderDualDecoderUNet(hparams)
